...
namespace ProlexNetSetup.Library
{
    internal class Download
    {
        ...
        // ProlexNet
        public static async Task ProlexNetAsync(Action<DownloadProgressChangedEventArgs, double> callback, string prolexNetServerPort)
        {
            var installationPath = MainWindowViewModel.InstallationPath;
            var servicePath = MainWindowViewModel.ServicePath;

            var url = DownloadParameters.AppList.ProlexNet_Url;
            var hash = DownloadParameters.AppList.ProlexNet_Hash;
        ...
        public static async Task DatabaseAsync(Action<DownloadProgressChangedEventArgs, double> callback)
        {
            ...
            var url = DownloadParameters.AppList.Database_Url;
            var downloadFileName = Path.GetFileName(url);
            var file = Path.Combine(servicePath, downloadFileName);
            var hash = DownloadParameters.AppList.Database_Hash;
        ...
        public static async Task NetCore21Async(Action<DownloadProgressChangedEventArgs, double> callback)
        {
            ...
            var url = DownloadParameters.AppList.NetCore21_Url;
            var hash = DownloadParameters.AppList.NetCore21_Hash;
